...
def _has_weather_intent(doc) -> bool:
    """
    Returns True if the doc contains a weather-related question or
    a request verb (imperative) in REQUEST_VERBS.
    Loops through sentences to safely access sentence roots.
    """
    ...
    for sent in doc.sents:
        # 2. Interrogative auxiliary attached to sentence root
        for token in sent:
            if (
                token.dep_ == "aux"
                and token.head == sent.root
                and token.lemma_ in {"be", "do", "will", "can", "should"}
            ):
                return True

        # 3. Imperative — only allowed if root verb is in REQUEST_VERBS
